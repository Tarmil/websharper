...
type TransformVarSets(v, tr) =
    ...
    override this.TransformVarSet(a, b) =
        if a = v then
             tr (this.TransformExpression' b) |> VSome
...
type MarkApplicationsPure(v, purity) =
    ...
    override this.TransformApplication(func, args, isPure, length) =
        match func with
        | I.Var x when x = v ->
            Application(func, args |> List.map this.TransformExpression', purity, length) |> VSome
        | _ -> base.TransformApplication(func, args, isPure, length)

    override this.TransformLet(id, value, body) =
        match value with
        | I.Var x when x = v ->
            ReplaceId(id, v).TransformExpression'(body) |> this.TransformExpression' |> VSome
...
let optimize expr =
    ...
    | Application(I.ItemGet(I.Function (vars, I.Return body), I.Value (String "apply"), _), [ I.Value Null; argArr ], _, _) ->
        List.foldBack2 bind vars (List.init vars.Length (fun i -> argArr.[Value (Int i)])) body |> VSome
    ...
    | Application (I.Sequential (_ :: _ :: _ as a), b, c, d) ->
        let ar = List.rev a
        Sequential (List.rev (Application (ar.Head, b, c, d) :: List.tail ar)) |> VSome
    // generated for disposing iterators
    | Application (ItemGet(Let (x, Var y, Var x2), i, gp), b, p, l) when x = x2 ->
        Application(ItemGet(Var y, i, gp), b, p, l) |> VSome
    | StatementExpr (I.ExprStatement a, None) ->
        Void a |> VSome
    | _ ->
        VNone

// TODO: should be more optimizeable wrt VOption
type Optimizer() =
    inherit Transformer()

    override this.TransformExpression (a) =
        let mutable i = 0
        let mutable a = base.TransformExpression(a).Or(a)
        let mutable b = a |> VOption.attempt removeLets |> VOption.attempt optimize |> Optimizations.cleanRuntime' false
#if DEBUG        
        ...
        while i < 10 && not (obj.ReferenceEquals(a, b) || a = b) do
            i <- i + 1
            a <- base.TransformExpression(b).Or(b)
            b <- a |> VOption.attempt removeLets |> VOption.attempt optimize |> Optimizations.cleanRuntime' false
#if DEBUG        
            if logTransformations then
...
//    match optimize expr with
                    ...
                    t |> List.choose (function ResultVar v -> Some v | _ -> None)
            {
                ...
                Statements = 
                    if List.isEmpty removeVars then
                        ...
                        brA.Statements |> VOption.bindList' (TransformMoreVarSets(removeVars, id).TransformStatement)
        ...
        | UnaryOperator.``void`` ->
            ...
            | ResultVar v ->
                { brB with
                    ...
                    Statements = brB.Statements |> VOption.bindList' (TransformVarSets(v, id).TransformStatement)
                }
            | ResultExpr e ->
                { brB with
    ...
    | StatementExpr (I.VarDeclaration(a, b), None) ->
        ...
        | ResultVar v ->
            {
                Body = ResultVar a
                Statements = brB.Statements |> VOption.bindList' (TransformVarSets(v, fun e -> VarSet(a, e)).TransformStatement)
    ...
    | Let(a, b, c) ->
        ...
        | ResultVar bv ->
            let brC = br c 
            {
                Body = brC.Body
                Statements = 
                    (brB.Statements |> VOption.bindList' (TransformVarSets(bv, fun e -> VarSet(a, e)).TransformStatement)) 
                    @ brC.Statements 
                Variables = (a, None) :: brB.Variables @ brC.Variables
            }
    | NewVar(a, Undefined) ->
